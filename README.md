...
macOS 桌面壳：用 Electron 原生窗口加载本机 [DeepSeek Harness](https://www.npmjs.com/package/@deepseek-ai/dsh)（`dsh web`）的本地 Web UI；打开即拉起 `dsh`，关闭窗口即退出壳并停止由其拉起的进程。
...
- **开即起、关即停**：进入壳时拉起 `dsh`；点 ✕ 关闭窗口时退出壳并停止本壳拉起的 `dsh`
- **独占端口**：若目标端口上已有 `dsh`，启动时先结束再由本壳重新拉起（以便拿到启动令牌）
...
| 能力 | 说明 |
|------|------|
| 自动拉起 | 启动时解析本机 `dsh`；若端口上已有 `dsh` 则先结束，再 `dsh web --no-open --port <port>` |
| Loading / 错误页 | 等待 HTTP 就绪时显示 loading；找不到 `dsh`、spawn 失败或超时可重试 |
| 托盘 | 运行中提供「显示 / 退出」；点窗口 ✕ 等同退出 |
...
│  - 清端口上旧 dsh → spawn / 停止      │
│  - Tray：显示窗口 / 退出               │
│  - close / quit → kill owned dsh     │
└──────────────┬──────────────────────┘
               │ HTTP 就绪后加载 ?token= URL
               ▼
┌─────────────────────────────────────┐
│  BrowserWindow                       │
...
| 变量 | 默认 | 说明 |
|------|------|------|
| `DSH_SKIN_PORT` | `18789` | `dsh web --port` 使用的端口；若该端口上已有 `dsh`，启动时会先结束再由本壳拉起 |
...
- **启动**：解析 `dsh` → 若端口上已有 `dsh` 则先结束 → `dsh web --no-open --port <port>` → 等待 HTTP 就绪后从 stdout 解析带 `?token=` 的启动 URL 并加载；等待期间显示 loading 页
- **健康探测**：运行中周期探测本地服务；不可达时显示错误页，可点击重试
- **加载失败 / 渲染崩溃**：先自动重载一次，仍失败则显示错误页
- **关窗 / 退出**：点窗口 ✕、托盘「退出」或 Ctrl+C 均退出壳，并停止本壳拉起的 dsh **进程组**（SIGTERM→SIGKILL）
