...
- **关窗**：点击窗口关闭按钮仅 **隐藏** 窗口，App 仍在菜单栏托盘运行，`dsh` 继续服务
- **唤回**：托盘「显示」或 Dock 图标点击可重新显示窗口
- **退出**：托盘「退出」时，若 dsh 由本壳 spawn，则 SIGTERM/SIGKILL 停止该子进程后再退出 Electron；若启动时复用了已有端口上的外部 `dsh`，退出时 **不会** 误杀外部进程
- **错误**：找不到 `dsh`、spawn 失败或超时等会显示错误页，可点击重试
